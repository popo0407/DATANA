...
- **方式**: API Gateway API Key + Usage Plan + IP アドレス制限
- **理由**:
  - ユーザー個別のログイン（Cognito）を介さずに認証が可能。
  - クライアントごとの流量制限（スロットリング）やクォータ設定が容易。
  - **IP アドレス制限**により、許可された特定のサーバー（社内サーバー等）以外からのアクセスを遮断し、セキュリティを強化する。

## 3. IP アドレス制限の仕様

- **設定方法**: インフラ（CloudFormation）のパラメータとして許可 IP 範囲（CIDR 形式）を定義。
- **デフォルト値**: `0.0.0.0/0`（初期設定では利便性のため全許可とするが、運用開始時に特定 IP に絞ることを推奨）。
- **判定ロジック**: Lambda の `requestContext` から送信元 IP を取得し、許可リストと照合する。
